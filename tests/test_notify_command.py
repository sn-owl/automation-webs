...
def _seed(root: Path, *, state: str) -> None:
    put_task(
        WorkItem.from_dict(
            {
                "task_id": "yeonje-1",
                "source": {"type": "gnuboard", "id": "yeonje", "external_id": "1", "url": "https://x.invalid/1"},
    ...
    log.append({"event_id": "yeonje-1:classified", "task_id": "yeonje-1", "stage": "classification",
                "state": "classified",
    ...
    log.append({"event_id": "yeonje-1:assessed", "task_id": "yeonje-1", "stage": "evaluation", "state": "assessed",
                "assessment": {"automation_level": "ready", "risk": "local_artifact_only", "confidence": 0.9,
                               "evidence": ["근거"], "recipe": "restarea-hwpx-to-xls"}})
    log.append({"event_id": f"yeonje-1:{state}", "task_id": "yeonje-1", "stage": "render", "state": state})
...
class NotifyCommandTest(unittest.TestCase):
    def test_review_required_task_is_queued_once_and_deduped(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            code, _, _ = run(["notify", "yeonje-1", "--root", str(root)])
            ...
            run(["notify", "yeonje-1", "--root", str(root)])
    ...
    def test_pipeline_events_create_immediate_notification_records_without_state_transition(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            _record_event(root, "pipeline-stored", "stored", task_id="yeonje-1", stage="storage")
            _record_event(root, "pipeline-review", "review_required", task_id="yeonje-1", stage="render")
            deliveries = store.read_deliveries()
            code, output, error = run(["notify", "yeonje-1", "--root", str(root)])
        ...
        self.assertTrue(all(record["local_reference"] == "state/tasks/yeonje-1.json" for record in deliveries))
    ...
    def test_task_not_awaiting_a_decision_is_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            code, _, err = run(["notify", "yeonje-1", "--root", str(root)])
