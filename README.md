...
여러 게시판·메신저·메일로 흩어져 들어오는 반복 업무를 **같은 계약으로 인식하고, 분류하고,
...
지자체 유지보수 게시판 업무를 예로 들면, 담당자는 매일 이런 일을 반복한다.

1. 여러 구청 게시판을 열어 새 요청이 올라왔는지 확인한다
2. 요청을 읽고 우리 담당인지, 어느 유형인지 판단한다
3. 반복되는 유형(예: 무더위쉼터 현황 HWPX → 업로드용 XLS 변환)은 손으로 같은 변환을 되풀이한다
4. 처리 결과를 기록하고 알린다
...
automation/            Core 패키지 — 분류·평가·패턴·Recipe·실행·검증·알림 (53개 모듈)
  ├─ adapters/         게시판별 입력 어댑터 (gnuboard, dongnae)
...
extension/             Chrome Extension — 게시판 수집기 (S1 입력 계층)
